...
export function HeroSlider({ slides }: { slides: ReactNode[] }) {
  ...
  return (
    <div
      // ~75-80% of the viewport height instead of nearly full-screen —
      // leaves a bit of the next section visible below the fold, reads
      // more balanced than an edge-to-edge banner.
      className="relative h-[65vh] w-full overflow-hidden sm:h-[75vh] md:h-[80vh]"
