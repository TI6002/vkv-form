...
export function AdminCollectionPanel() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      // The name is deliberately NOT auto-translated — DeepL sometimes
      // mistranslates short names, or inserts a non-breaking space
      // around punctuation that later breaks mobile layout (a long
      // "unbreakable" run of text). It's set per language directly
      // below in "Set the name in each language", same as the regular
      // catalogue products.
      ...
      if (failedLocales.length > 0) {
        ...
        alert(
          `Saved — but translating the description/materials/dimensions failed for: ${failedLocales.join(', ')}.\n\n` +
    ...
    <div className="mt-10 grid gap-16 lg:grid-cols-[1fr_1.2fr]">
      ...
      <div>
        ...
        <p className="mt-2 max-w-sm font-body text-xs leading-relaxed text-taupe">
          ...
          translated automatically from whatever language you pick here.
        </p>
        <form onSubmit={handleSave} className="mt-6 flex flex-col gap-5">
          ...
          <p className="-mt-2 font-body text-xs text-taupe">
            Press Enter twice for a new paragraph. Spaces and tabs you
            type are preserved on the item page (e.g. for indenting a
            line).
          </p>
          <label className="block">
            <span className="font-mono text-[11px] uppercase tracking-widest2 text-stone">
        ...
        <div className="mt-12 border-t border-line pt-8">
          ...
          <p className="mt-2 max-w-sm font-body text-xs leading-relaxed text-taupe">
            ...
            will fall back to whatever you typed in &quot;Name&quot; above.
          </p>
          ...
          {editingId ? (
            ...
            <p className="mt-5 font-body text-xs text-taupe">
              ...
              the form when you click &quot;Save item&quot; above for a
              new item.
            </p>
          )}
        </div>
