...
export function AdminCollectionPanel() {
  ...
  /**
   * Moves a piece back from the Collection Book to the regular
   * catalogue — the reverse of what the Stripe webhook does when a
   * piece sells. Collection Book entries don't store a price, slug, or
   * stock (those only exist on products), so those are asked for here
   * and generated fresh rather than guessed at.
   */
  async function handleRestoreToCatalog(item: CollectionItem) {
    const englishName = pickLocalized(item.name, 'en') || pickLocalized(item.name, locale);
    const priceInput = window.prompt(
      `Move "${englishName}" back to the catalogue.\n\nEnter its price in EUR (numbers only, e.g. 850):`
    );
    if (priceInput === null) return; // cancelled

    const priceEur = parseFloat(priceInput.replace(',', '.'));
    if (!priceEur || priceEur <= 0) {
      alert('Please enter a valid price greater than 0.');
      return;
    }

    const baseSlug = slugify(englishName);
    let slug = baseSlug;
    let attempt = 0;
    // Guard against colliding with an existing product slug.
    ...
    if (insertError) {
      ...
      alert('Could not move this item back to the catalogue — check the console.');
      return;
    }
    ...
    if (deleteError) {
      ...
      alert(
        'Added back to the catalogue, but could not remove it from the Collection Book — please delete it there manually.'
      );
    }
            ...
            {items.map((item) => (
              <li key={item.id} className="flex items-center justify-between gap-4 py-4">
                ...
                <div className="flex gap-4">
                  <button
                    onClick={() => handleRestoreToCatalog(item)}
