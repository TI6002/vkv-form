...
type Section = 'catalog' | 'orders' | 'about' | 'collection';

export function AdminDashboard() {
  const [section, setSection] = useState<Section>('catalog');

  // Orders is now its own top-level tab — same level as Catalog, About
  // page and Collection Book — rather than a toggle button that only
  // appeared while inside Catalog. That toggle used to vanish the
  // moment you switched to About or Collection, making Orders
  // unreachable from there without first clicking back into Catalog.
