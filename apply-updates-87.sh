...
echo "Applying vkv.form updates — fix sort order mismatch (renumber all items on every move)..."
...
echo ""
echo "Done. git add -A && git commit -m \"Fix sort order mismatch: renumber all items sequentially on every move\" && git push"